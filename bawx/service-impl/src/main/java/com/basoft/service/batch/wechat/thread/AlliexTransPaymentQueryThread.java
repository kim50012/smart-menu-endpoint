 ...
 * 获取关注用户线程
 */
public class AlliexTransPaymentQueryThread implements Runnable {
	 ...
	 */
	public AlliexTransPaymentQueryThread(WechatPayAlliexTransService wechatPayAlliexTransService) {
		this.wechatPayAlliexTransService = wechatPayAlliexTransService;
	}

	@Override
	public void run() {
		logger.info("++++++++++++++++ AlliexTransPaymentQueryThread.run() ++++++++++++++++");
		Date ifStartDate = new Date(new Date().getTime() - 5 * 60 * 1000);
		logger.info(ifStartDate);

		Map<String,Object> map = new HashMap<String, Object>();

		logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0100 : Create Batch Header Id ++++++++++++++++");
		...
		try {
			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0200 : Create OrderPayTrans Data ++++++++++++++++");
			// 알리엑스 I/F 대상이 되는 데이터 생성
			map.clear();
			wechatPayAlliexTransService.insertBatchInitial_order_pay_trans_search(map);


			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0300 : Update Waite status Max 100 low ++++++++++++++++");
			// 알리엑스 I/F 대기 상태로 데이터 업데이트 한번에 100건
			map.clear();
			map.put("batch_id", batch_id);
			wechatPayAlliexTransService.updateTargetWait_order_pay_trans_search(map);

			
			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0400 : Get I/F Data ++++++++++++++++");
			// 알리엑스 I/F 되는 대상 데이터 조회
			map.clear();
			map.put("batch_id", batch_id);
			List<Map<String, Object>> alliPaymentList = wechatPayAlliexTransService.selectTargetList_order_pay_trans_search(map);

			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0410 : I/F Data Total Count ▶▶▶▶ " + alliPaymentList.size());
			for(int i = 0; i < alliPaymentList.size(); i++){
				Map<String,Object> alliPaymentData = alliPaymentList.get(i);
				int x = i + 1;
				logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0500 : Post I/F Data  ▶▶ " + x + " th ◀◀");
			...
			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread - Update Batch Run Result ++++++++++++++++");
			...
			logger.error("++++++++++++++++ AlliexTransPaymentQueryThread - Error : " + e.getMessage());
	...
	private void trasToAlliex(Map<String,Object> alliPaymentData){
		...
		long search_id = (long) alliPaymentData.get("search_id");
		...
		logger.info("++++++++++++++++ AlliexTransPaymentQueryThread -STEP 0510 : secretValue : " + secretValue);
		logger.info("▶▶▶▶▶▶▶▶▶▶▶▶▶▶  AlliexTransPaymentQueryThread -STEP 0511 : trans_id : " + alliPaymentData.get("trans_id").toString());
		...
		try {
			...
			String alliexIfUrl = WechatPayAlliexUtils.ALLIEX_PAYMENT_QUERY_URL;
			if ("BA".equals(transid_type)) {
				alliexIfUrl = WechatPayAlliexUtils.ALLIEX_PAYMENT_QUERY_URL_LIVE;
			}
			...
			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread -STEP 0520 : Result postData : \n" + postData);
			...
			*/
			...
			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread -STEP 0530 : Result Data : ");
			...
			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread -STEP 0540 : Check ResponseCode ++++++++++++++++");
			if (!r_responsecode.equals("0000")) {
				status = 9;
			}
			

			logger.info("++++++++++++++++ AlliexTransPaymentQueryThread -STEP 0550 : Save Result Data : ++++++++++++++++");
			// I/F 결과 저장 (정상처리)
			map.clear();
			...
			try {
				wechatPayAlliexTransService.updateBatchResult_order_pay_trans_search(map);
			}
			catch (Exception e) {
				logger.error("++++++++++++++++ AlliexTransPaymentQueryThread - STEP 0551 trasToAlliex Update Error : " + e.getMessage());
			}
		} catch (Exception e) {
			logger.error("++++++++++++++++ AlliexTransPaymentQueryThread - trasToAlliex Error : " + e.getMessage());
			// I/F 결과 저장 (오류처리)
			map.clear();
			map.put("search_id", search_id);
			map.put("trans_id", trans_id);
