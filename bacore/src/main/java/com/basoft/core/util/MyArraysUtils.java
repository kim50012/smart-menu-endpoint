...
/**
 * ClassName:MyArraysUtils 数组处理类<br/>
 * Date: 2017年7月7日 下午3:49:31 <br/>
 * 
 * @version
 * @see
 */
public class MyArraysUtils {
	...
	/**
	 * transformListToArray:(将List转成Object数组). <br/>
	 * 
	 * @author zhiyong.li
	 * @param list
	 * @return
	 */
	public Object[] transformListToArray(List<?> list) {
		return list.toArray();
	}

	/**
	 * transformListToArray2:(将List转换成特定的数组). <br/>
	 * 
	 * @author zhiyong.li
	 * @param list
	 * @param t
	 * @return
	 */
	public <T> T[] transformListToArray2(List<?> list, T[] t) {
		return list.toArray((T[]) t);
	}
	...
	/**
	 * bubbleSort:(冒泡排序 方法：相邻两元素进行比较 性能：比较次数O(n^2),n^2/2；交换次数O(n^2),n^2/4 ).
	 * <br/>
	 * 
	 * @author zhiyong.li
	 * @param source
	...
	/**
	 * selectSort:(直接选择排序法 方法：每一趟从待排序的数据元素中选出最小（或最大）的一个元素，
	 * 顺序放在已排好序的数列的最后，直到全部待排序的数据元素排完。 ). <br/>
	 * 
	 * <pre>
	...
	/**
	 * insertSort:(插入排序 方法：将一个记录插入到已排好序的有序表（有可能是空表）中,从而得到一个新的记录数增1的有序表。
	...
	/**
	 * quickSort:(快速排序 快速排序使用分治法（Divide and
	...
	/**
	 * binarySearch:(二分法查找 查找线性表必须是有序列表 ). <br/>
	 * 
	 * @author zhiyong.li
	 * @param source
	 *            需要进行查找操作的数组
	...
	/**
	 * insert:(在当前位置插入一个元素,数组中原有元素向后移动; ). <br/>
	...
	/**
	 * remove:(整形数组中特定位置删除掉一个元素,数组中原有元素向前移动; ). <br/>
	 * 如果插入位置超出原数组，则抛IllegalArgumentException异常
	 * 
	 * @author zhiyong.li
	 * @param array
	...
	/**
	 * offsetArray:(数组中有n个数据，要将它们顺序循环向后移动k位，即前面的元素向后移动k位，后面的元素则循环向前移k位， ). <br/>
	 * 
	 * <pre>
	...
	/**
	 * mergeByList:(对两个有序数组进行合并,并将重复的数字将其去掉 ). <br/>
	 * 
	 * @author zhiyong.li
	 * @param a
	 *            已排好序的数组a
