 ...
 */
@Slf4j
public class AgentSAMonthSettlementThread implements Runnable {
    ...
    public AgentSAMonthSettlementThread(IAgentBatchService agentBatchService
    ...
    public void run() {
        try {
            agentBatchService.storeAgentMonthSettle(agent, lastYear, lastMonth, year, month, monthStartTime, monthEndTime);
        } catch (Exception e) {
            log.error("【SA代理商月度结算】SA代理商结算异常，异常信息为：" + e.getMessage(), e);
        }
    }
}
