    ...
    public static <T> void writeExcel(HttpServletResponse response,String describle,List<String> topList, List<T> dataList, Class<T> cls,String fileName,String language){
        ...
        AtomicInteger ai = new AtomicInteger();
        {
            ...
            if(CollectionUtils.isNotEmpty(topList)) {
                Row rowTop = sheet.createRow(ai.getAndIncrement());
                AtomicInteger ajTop = new AtomicInteger();
                AtomicInteger num = new AtomicInteger(1);
                topList.forEach(t -> {
                    Cell cell = rowTop.createCell(ajTop.getAndIncrement());
                    CellStyle cellStyle = wb.createCellStyle();
                    if (num.getAndIncrement()%2!=0){
                    ...
                    //cellStyle.setBorderBottom((short) 1);
                    cellStyle.setRightBorderColor(IndexedColors.GREY_25_PERCENT.getIndex());
                    sheet.setColumnWidth(ajTop.get(),256*30);
    ...
    public static  String getFileName(String language, String titleStart,String type) {
        ...
        if(type.equals(STORESETTLES_TYPE)){
            if (language.equals("kor")) {
                fileName += titleStart + ExcelUtils.FILENAME_STORESETTLES_KOR;
            } else {
                fileName += titleStart + ExcelUtils.FILENAME_STORESETTLES_CHN;
            }
        } else if (type.equals(AGT_SETTLES_TYPE)) {
            if (language.equals("kor")) {
                fileName += titleStart + ExcelUtils.FILENAME_AGT_SETTLES_KOR;
            } else {
                fileName += titleStart + ExcelUtils.FILENAME_AGT_SETTLES_CHN;
            }
        }
        return fileName;
    }
